...
export function YandexCallbackClient() {
  ...
  useEffect(() => {
    ...
    const savedState = localStorage.getItem(YANDEX_OAUTH_STATE_KEY);

    if (!code || !state || state !== savedState) {
      router.replace("/?error=yandex_state");
      return;
    }
    ...
    void (async () => {
      ...
      if (!API_URL) {
        router.replace("/?error=yandex_signin");
        return;
      }

      const res = await fetch(`${API_URL}/auth/yandex/sign-in`, {
        method: "POST",
  ...
  return <div>Вход через Яндекс...</div>;
}
