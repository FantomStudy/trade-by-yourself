...
function getCityFromAddress(address?: string | null) {
  ...
  const explicitPatterns = [
    // г. Оренбург или г Оренбург — после "г" должна быть точка или пробел, а не буква
    /(?:^|,\s*)г[.\s]\s*([А-ЯA-ZЁ][А-ЯA-ZЁа-яa-zё\-\s]+?)(?:\s*,|$)/i,
...
export const SecureDealForm = ({ product }: SecureDealFormProps) => {
  ...
  const resetState = () => {
    ...
    setToPvzCode("");
    setToAddress("");
    setTariffName(WAREHOUSE_TO_WAREHOUSE_TARIFF_NAME);
    setTariffCode(WAREHOUSE_TO_WAREHOUSE_TARIFF_CODE);
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const result = await calculateCdekDelivery({
        tariffCode: WAREHOUSE_TO_WAREHOUSE_TARIFF_CODE,
        fromCityCode,
        toCityCode,
        weight: parsedWeight,
        length: parsedLength,
