...
class Fetches {
  ...
  private async request<Data, R = FetchesResponse<Data>>(
    ...
    const { body, query, parse, baseURL: baseUrl, ...rest } = options;
    ...
    // С этого момена ЗАМЕНА НА config

    const response: Response = await fetch(defaultConfig.url, defaultConfig);

    // if (this.interceptorHandlers.response?.length) {
    //   return this.runResponseInterceptors<Data>(response, config, options) as R;
    // }

    const data = await this.parseResponse<Data>(response, defaultConfig.parse);
